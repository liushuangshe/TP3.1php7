	...
	function ParseCode(
		...
		//分析图片地址
 		...
 		$ShowTPL = str_replace('$','\$',$ShowTPL);
			...
			//兼容php7修复
			// By Legolas 2016-06-27 15:30
			$ShowTPL = @preg_replace_callback("/\";(.+?)echo\"/", function ($match){$this->FixPHP($match[1]);}, $ShowTPL);
			
			//Fix Run 2
			if ($run==1){
				//$ShowTPL = preg_replace("/\(T_T\)(.+?)\(T_T!\)/ise", '$this->FixPHP(\'\\1\')', $ShowTPL);
				//兼容php7修复
				// By Legolas 2016-06-27 15:34
				$ShowTPL = preg_replace_callback("/\(T_T\)(.+?)\(T_T!\)/is", function ($match){$this->FixPHP($match[1]);}, $ShowTPL);
			}
