...
class  ThinkTemplate {
    ...
    protected function compiler($tmplContent) {
		...
		//兼容php7的修改
		// By Legolas 2016-06-27 16:38
		$tmplContent =  preg_replace_callback('/<!--###literal(\d+)###-->/is',function ($match){return $this->restoreLiteral($match[1]);},$tmplContent);
        // 添加安全代码
    ...
    public function parse($content) {
		...
		//兼容php7的修改
		// By Legolas 2016-06-27 00:09
		$content = preg_replace_callback('/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/is',function ($match){$this->parseLiteral($match[1]);},$content);
		...
		//兼容php7的修改
		// By Legolas 2016-06-28 01:04
		$content = preg_replace_callback('/('.$this->config['tmpl_begin'].')([^\d\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/is',function ($match){return $this->parseTag($match[2]);},$content);
        return $content;
    }
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
			...
			//兼容php7的修改 
			// By Legolas 2016-06-26 23:54
			$content = preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is',function ($match){return stripslashes($match[1]);},$content);            
        }
        return $content;
    }
